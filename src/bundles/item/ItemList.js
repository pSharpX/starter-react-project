...
export class ItemList extends Component {
  ...
  render () {
    ...
    const Collection = items.map((item, key) => (
      <Item key={item.Id} ViewMode={viewMode} Input={item} />
    ))
    ...
    const viewModeToolbar = (
      <div className="d-flex justify-content-end mb-2">
        ...
        <div className="btn-group btn-group-sm btn-group-toggle">
          <label
            className={`btn btn-success ${this.state.option1 ? 'active' : ''}`}
            htmlFor="option1"
          >
            <input
              type="radio"
              ...
              id="option1"
              value="table-view"
              onChange={this.handlerViewModeChange}
              checked={this.state.option1}
            />
            <FontAwesomeIcon icon="table" />
          </label>
          <label
            className={`btn btn-success ${this.state.option2 ? 'active' : ''}`}
            htmlFor="option2"
          >
            <input
              type="radio"
          ...
          <label
            className={`btn btn-success ${this.state.option3 ? 'active' : ''}`}
            htmlFor="option3"
          >
            <input
              type="radio"
    ...
    if (viewMode === 'card-view') viewContainer = cardViewContainer
    else if (viewMode === 'table-view') viewContainer = tableViewContainer
    else if (viewMode === 'list-view') viewContainer = listViewContainer
    else viewContainer = cardViewContainer
    return (
